...
step_result = cleaned.gsub(/(\d)\/Conformance(\d+)(?!-\d{4})/, '\1 /Conformance\2')
puts "Before line 971: #{cleaned}"
puts "After line 971: #{step_result}"
puts "Match: #{cleaned.match(/(\d)\/Conformance(\d+)(?!-\d{4})/).inspect rescue 'no match'}"

# Also test a malformed pattern (should get space added)
malformed = "1904.1(TM)/Conformance02"
malformed_result = malformed.gsub(/(\d)\/Conformance(\d+)(?!-\d{4})/, '\1 /Conformance\2')
