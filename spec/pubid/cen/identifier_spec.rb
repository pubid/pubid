  ...
  describe "Pubid::Cen.parse" do
    context "European Norm (EN) identifiers" do
      [
        ...
        it "round-trips #{pubid}" do
          identifier = Pubid::Cen.parse(pubid)
          expect(identifier.to_s).to eq(pubid)
          expect(identifier).to be_a(Pubid::Cen::Identifiers::EuropeanNorm)
        end
      end
    end
    ...
    context "Technical Specification (TS)" do
      it "round-trips CEN/TS" do
        identifier = Pubid::Cen.parse("CEN/TS 1234:2010")
        expect(identifier.to_s).to eq("CEN/TS 1234:2010")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::TechnicalSpecification)
      end
    end

    context "Technical Report (TR)" do
      it "round-trips CEN/TR" do
        identifier = Pubid::Cen.parse("CEN/TR 456:2015")
        expect(identifier.to_s).to eq("CEN/TR 456:2015")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::TechnicalReport)
      end
    end

    context "CEN Workshop Agreement (CWA)" do
      it "round-trips CWA" do
        identifier = Pubid::Cen.parse("CWA 17145-2:2017")
        expect(identifier.to_s).to eq("CWA 17145-2:2017")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::CenWorkshopAgreement)
      end
    end

    context "copublished documents" do
      it "round-trips EN/CLC/TS" do
        identifier = Pubid::Cen.parse("EN/CLC/TS 50131-1:2006")
    ...
    context "bundled identifiers (+ operator)" do
      it "parses single bundled corrigendum" do
        identifier = Pubid::Cen.parse("EN 10077-1:2006+AC:2009")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::ConsolidatedIdentifier)
        expect(identifier.to_s).to eq("EN 10077-1:2006+AC:2009")
        expect(identifier.identifiers.first).to be_a(Pubid::Cen::Identifiers::EuropeanNorm)
        expect(identifier.identifiers.length).to eq(2)  # base + 1 supplement
        expect(identifier.identifiers.last).to be_a(Pubid::Cen::Identifiers::Corrigendum)
      end

      it "parses multiple bundled supplements" do
        identifier = Pubid::Cen.parse("EN 10077-1:2006+AC:2009+AC2:2009")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::ConsolidatedIdentifier)
    ...
    context "slash supplements (/ operator)" do
      it "parses and renders amendment" do
        identifier = Pubid::Cen.parse("EN 1234:1999/A1:2005")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::Amendment)
        expect(identifier.to_s).to eq("EN 1234:1999/A1:2005")
        expect(identifier.base_identifier).to be_a(Pubid::Cen::Identifiers::EuropeanNorm)
        expect(identifier.amendment_number).to eq("1")
      end

      it "parses and renders corrigendum" do
        identifier = Pubid::Cen.parse("EN 1234:1999/AC1:2005")
        expect(identifier).to be_a(Pubid::Cen::Identifiers::Corrigendum)
