...
module CenFixturesSpec
  # __dir__ is spec/pubid/cen_cenelec, so the fixtures are two levels up, not
  # three. The old path ("../../../fixtures/cen_cenelec") pointed at a
  # non-existent repo-root fixtures/ and made this whole spec a no-op: it
  # iterated an empty file list and reported 0 examples.
  FIXTURES_DIR = File.expand_path("../../fixtures/cen_cenelec/identifiers",
                                  __dir__)
...
RSpec.describe "CEN/CENELEC fixture round-trip" do
  include FixtureFileHelper

  def passing_inputs
    CenFixturesSpec::PASS_FILES.flat_map do |file|
      read_pass_fixture_entries(file).map(&:first)
    end
  end

  def failing_inputs
    CenFixturesSpec::FAIL_FILES.flat_map do |file|
      read_fail_fixture_inputs(file)
    end
  end

  # The tripwire. Every example below is generated from a glob, so a wrong
  # path yields zero examples and zero failures. This example fails instead.
  it "finds the generated fixture files" do
    expect(CenFixturesSpec::PASS_FILES).not_to be_empty
    expect(CenFixturesSpec::FAIL_FILES).not_to be_empty
    expect(CenFixturesSpec::FULL_FILES).not_to be_empty
  end

  CenFixturesSpec::PASS_FILES.each do |fixture_file|
    describe "pass/#{File.basename(fixture_file)}" do
      it "parses every identifier and renders it as recorded" do
        ...
        failures = entries.filter_map do |input, expected|
          rendered = Pubid::CenCenelec.parse(input).to_s
        ...
        expect(failures).to be_empty,
                            "CEN pass fixtures did not round-trip: " \
                            "#{failures.first(10).inspect}"
      end
    end
  end

  CenFixturesSpec::FAIL_FILES.each do |fixture_file|
  ...
  # full/*.txt, so the two sides must account for exactly the same
  ...
  it "classifies every identifier in full/, and no others" do
    source = CenFixturesSpec::FULL_FILES.flat_map do |file|
      read_pass_fixture_entries(file).map(&:first)
    end
  ...
  # sides, so the partition is asserted separately.
