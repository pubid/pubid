  ...
  describe ".parse" do
    context "basic IR identifiers" do
      ...
      describe "NIST IR 84-2946" do
        ...
        it "round-trips" do
          expect(parsed.to_s).to eq("NIST IR 84-2946")
        end
      end
    ...
    context "IR with update" do
      describe "NISTIR 8115r1/upd" do
        ...
        it "normalizes to update format" do
          # Input has no year/month, so only update number is rendered
          expect(parsed.to_s).to eq("NIST IR 8115r1/Upd1")
        end
        ...
        it "parses update" do
          ...
          # No year/month in input, so these should be nil
          expect(parsed.update.year).to be_nil
          expect(parsed.update.month).to be_nil
        end
      end
    ...
    context "IR with parts" do
      ...
      describe "NBS IR 74-577-1" do
        ...
        it "normalizes to part notation" do
          expect(parsed.to_s).to eq("NBS IR 74-577pt1")
        end

        it "parses as part" do
          expect(parsed.part.value).to eq("1")
        end
      end
    end
