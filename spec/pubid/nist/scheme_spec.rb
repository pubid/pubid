...
RSpec.describe Pubid::Nist::Scheme do
  ...
  describe ".locate_typed_stage_by_abbr" do
    it "finds stage by simple abbreviation" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("SP")
    ...
    it "finds stage by compound abbreviation with publisher" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("NIST SP")
      expect(stage.type_code).to eq("sp")
    end

    it "finds stage by NBS compound abbreviation" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("NBS SP")
      expect(stage.type_code).to eq("sp")
    end

    it "finds FIPS stage" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("FIPS")
      expect(stage.type_code).to eq("fips")
    end

    it "finds Handbook stage" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("HB")
      expect(stage.type_code).to eq("hb")
    end

    it "finds Technical Note stage" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("TN")
      expect(stage.type_code).to eq("tn")
    end

    it "finds Circular stage" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("CIRC")
      expect(stage.type_code).to eq("circ")
    end

    it "finds CRPL Report stage" do
      stage = Pubid::Nist::Scheme.locate_typed_stage_by_abbr("CRPL")
      expect(stage.type_code).to eq("crpl")
    end

    it "raises ArgumentError for unknown abbreviation" do
      expect do
        Pubid::Nist::Scheme.locate_typed_stage_by_abbr("UNKNOWN")
  ...
  describe ".locate_identifier_klass_by_type_code" do
    it "returns SpecialPublication class for sp type code" do
      klass = Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("sp")
      expect(klass).to eq(Pubid::Nist::Identifiers::SpecialPublication)
    end

    it "returns FederalInformationProcessingStandards class for fips type code" do
      klass = Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("fips")
      expect(klass).to eq(Pubid::Nist::Identifiers::FederalInformationProcessingStandards)
    end

    it "returns Handbook class for hb type code" do
      klass = Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("hb")
      expect(klass).to eq(Pubid::Nist::Identifiers::Handbook)
    end

    it "returns TechnicalNote class for tn type code" do
      klass = Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("tn")
      expect(klass).to eq(Pubid::Nist::Identifiers::TechnicalNote)
    end

    it "returns Circular class for circ type code" do
      klass = Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("circ")
      expect(klass).to eq(Pubid::Nist::Identifiers::Circular)
    end

    it "returns CrplReport class for crpl type code" do
      klass = Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("crpl")
      expect(klass).to eq(Pubid::Nist::Identifiers::CrplReport)
    end

    it "raises ArgumentError for unknown type code" do
      expect do
        Pubid::Nist::Scheme.locate_identifier_klass_by_type_code("unknown")
