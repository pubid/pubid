  ...
  describe "#parse" do
    context "basic EN identifiers" do
      ...
      it "parses EN with date" do
        ...
        expect(result[:publisher]).to eq("EN")
        expect(result[:number_with_part]).to eq("10077-1")
        expect(result[:date]).to eq("2006")
      end

      it "parses multi-part numbers" do
        result = subject.parse("EN 29110-5-1-1:2015")
        expect(result[:number_with_part]).to eq("29110-5-1-1")
      end
    end
    ...
    context "stage prefixes" do
      it "parses prEN (proposal)" do
        ...
        expect(result[:number_with_part]).to eq("15316-1")
      end

      it "parses FprEN (final proposal)" do
        result = subject.parse("FprEN 987:2018")
        expect(result[:type_with_stage][:stage]).to eq("Fpr")
        expect(result[:number_with_part]).to eq("987")
      end
    end
    ...
    context "typed documents" do
      it "parses CEN TS" do
        ...
        expect(result[:number_with_part]).to eq("1234")
      end

      it "parses CEN TR" do
        result = subject.parse("CEN TR 456:2015")
        expect(result[:publisher]).to eq("CEN")
        expect(result[:type_with_stage]).to eq("TR")
      end

      it "parses CWA" do
        result = subject.parse("CWA 17145-2:2017")
        expect(result[:type_with_stage]).to eq("CWA")
        expect(result[:number_with_part]).to eq("17145-2")
      end
    end
    ...
    context "copublished documents" do
      it "parses EN/CLC" do
        ...
        expect(result[:copublishers].first[:copublisher]).to eq("CLC")
        expect(result[:type_with_stage]).to eq("TS")
      end
    end
    ...
    context "bundled identifiers" do
      it "parses single bundled corrigendum" do
        ...
        expect(result[:base_document]).to be_a(Hash)
    ...
    context "slash supplements" do
      it "parses amendment" do
        ...
        expect(result[:type_with_stage]).to eq("A")
        expect(result[:supplement_number]).to eq("1")
      end

      it "parses corrigendum" do
        result = subject.parse("EN 1234:1999/AC1:2005")
        expect(result[:base_identifier]).to be_a(Hash)
        expect(result[:type_with_stage]).to eq("AC")
        expect(result[:supplement_number]).to eq("1")
      end
    end
  end
