  ...
  describe ".parse" do
    context "publisher patterns" do
      ...
      it "parses ISO/SAE copublisher" do
        result = described_class.parse("ISO/SAE PAS 22736:2021")
        expect(result[:base]).to be_an(Array)
        expect(result[:base]).to include(hash_including(publisher: "ISO"))
      ...
      it "parses ISO/ASTM copublisher" do
        result = described_class.parse("ISO/ASTM 51276:2020")
        expect(result[:base]).to include(hash_including(copublisher: "ASTM"))
      end

      it "parses ISO/CIE copublisher" do
        result = described_class.parse("ISO/CIE 11664-1:2019")
    ...
    context "type patterns" do
      ...
      it "parses TS" do
        result = described_class.parse("ISO/IEC TS 25011:2017")
      ...
      it "parses DATA" do
        result = described_class.parse("ISO/DATA 7:1979")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:type]).to eq("DATA")
      end

      it "parses DIR" do
        result = described_class.parse("ISO/IEC DIR 1:2022")
        expect(result[:base]).to include(hash_including(type: "DIR"))
      end

      it "parses ISP" do
        result = described_class.parse("ISO/IEC ISP 12062-2:2003")
      ...
      it "parses TTA" do
        result = described_class.parse("ISO/TTA 5:2007")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:type]).to eq("TTA")
      end

      it "parses R (legacy Recommendation)" do
        result = described_class.parse("ISO/R 300-3:1968")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:type]).to eq("R")
      end

      it "parses Guide" do
        result = described_class.parse("ISO/IEC Guide 51:1999")
    ...
    context "typed stage patterns" do
      ...
      it "parses PDAM" do
        result = described_class.parse("ISO 12345:2020/PDAM 2")
        supp = result[:supplements].first
        expect(supp[:typed_stage][:typed_stage]).to eq("PDAM")
      end

      it "parses DAM" do
        result = described_class.parse("ISO 12345:2020/DAM 1")
        supp = result[:supplements].first
        expect(supp[:typed_stage][:typed_stage]).to eq("DAM")
      end

      it "parses FDCOR" do
        result = described_class.parse("ISO 12345:2020/FDCOR 1")
        supp = result[:supplements].first
        expect(supp[:typed_stage][:typed_stage]).to eq("FDCOR")
      end

      it "parses DCOR" do
        result = described_class.parse("ISO 12345:2020/DCOR 1")
        supp = result[:supplements].first
        expect(supp[:typed_stage][:typed_stage]).to eq("DCOR")
      end

      it "parses FDIS" do
        result = described_class.parse("ISO FDIS 19115:2003")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:typed_stage]).to eq("FDIS")
      end

      it "parses DIS" do
        result = described_class.parse("ISO DIS 19115:2003")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:typed_stage]).to eq("DIS")
      end

      it "parses DTR" do
        result = described_class.parse("ISO/IEC DTR 29186:2012")
        expect(result[:base]).to include(hash_including(typed_stage: "DTR"))
      end

      it "parses DTS" do
        result = described_class.parse("ISO/IEC DTS 25011:2017")
    ...
    context "supplement patterns" do
      ...
      it "parses Cor with number and year" do
        result = described_class.parse("ISO/IEC 8802-21:2018/Cor 1:2018")
        supp = result[:supplements].first
      ...
      it "parses Suppl" do
        result = described_class.parse("ISO/TR 10000:2000/Suppl 1:2005")
        supp = result[:supplements].first
        expect(supp[:supplement_type]).to eq("Suppl")
      end

      it "parses Ext" do
        result = described_class.parse("ISO 1101:1983/Ext 1:1983")
        supp = result[:supplements].first
        expect(supp[:supplement_type]).to eq("Ext")
      end

      it "parses AMD (uppercase)" do
        result = described_class.parse("ISO 12345:2020/AMD 1:2021")
        supp = result[:supplements].first
        expect(supp[:supplement_type]).to eq("AMD")
      end

      it "parses COR (uppercase)" do
        result = described_class.parse("ISO 12345:2020/COR 1:2021")
        supp = result[:supplements].first
        expect(supp[:supplement_type]).to eq("COR")
      end
      ...
      it "parses supplement without year" do
        result = described_class.parse("ISO 12345:2020/Amd 1")
        supp = result[:supplements].first
        expect(supp[:supplement_type]).to eq("Amd")
    ...
    context "DIR SUP pattern" do
      ...
      it "parses directives supplement with IEC copublisher" do
        result = described_class.parse("ISO/IEC DIR 2 ISO/IEC SUP:2023")
        sup_pub_entry = result[:base].find { |h| h.key?(:sup_publisher) }
    ...
    context "number and parts" do
      ...
      it "parses with multiple parts" do
        result = described_class.parse("ISO 12345-1-2:2020")
        expect(result[:base][:parts]).to be_an(Array)
      ...
      it "parses alphanumeric part" do
        result = described_class.parse("ISO 12345-A01:2020")
    ...
    context "years and languages" do
      ...
      it "parses single language" do
        result = described_class.parse("ISO 19115:2003(E)")
        expect(result[:base]).to be_a(Hash)
      ...
      it "parses multiple languages with comma" do
        result = described_class.parse("ISO 19115:2003(E,F)")
    ...
    context "stage patterns" do
      it "parses WD stage" do
        result = described_class.parse("ISO WD 19115:2003")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:stage]).to eq("WD")
      end

      it "parses CD stage" do
        result = described_class.parse("ISO CD 19115:2003")
        expect(result[:base]).to be_a(Hash)
        expect(result[:base][:stage]).to eq("CD")
      end

      it "parses PWI stage" do
        result = described_class.parse("ISO PWI 19115")
        expect(result[:base]).to be_a(Hash)
    ...
    context "complex combinations" do
      it "parses identifier with all features" do
        result = described_class.parse("ISO/IEC TR 29186-1:2012(E/F)")
      ...
      it "parses three-level supplement" do
        result = described_class.parse("ISO 12345:2020/Amd 1:2021/Cor 1:2022/Amd 2:2023")
