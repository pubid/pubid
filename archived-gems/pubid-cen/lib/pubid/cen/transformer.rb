  ...
  module Cen
    class Transformer < Parslet::Transform
      ...
      rule(supplement: subtree(:supplement)) do |context|
        context[:supplement][:type] =
          convert_supplement_type(context[:supplement][:type])
        context
      end
      ...
      def self.convert_supplement(supplement)
        ...
        when "A"
          Identifier::Amendment.new(**supplement.dup.tap do |h|
            h.delete(:type)
          end)
        when "AC"
          Identifier::Corrigendum.new(**supplement.dup.tap do |h|
            h.delete(:type)
          end)
        end
      end
