    ...
    def initialize(config:, abbr: nil, harmonized_code: nil)
      ...
      if abbr
        if config.stages["project_stages"].key?(abbr.to_s)
          @harmonized_code ||= Pubid::Core::HarmonizedStageCode.new(
            config.stages["project_stages"][abbr.to_s], config: config
          )
          # update abbreviation with identifier's stage
          @abbr = lookup_abbr(@harmonized_code.stages)
        else
          unless config.stages["abbreviations"].key?(abbr.to_s)
            raise Pubid::Core::Errors::StageInvalidError,
                  "#{abbr} is not valid stage"
          end

          @harmonized_code ||= Pubid::Core::HarmonizedStageCode.new(
            lookup_code(abbr), config: config
          )
        end
      end
    end
