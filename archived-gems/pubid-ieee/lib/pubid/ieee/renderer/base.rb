    ...
    module Renderer
      class Base < Pubid::Core::Renderer::Base
        ...
        def render_year(year, _opts, params)
          ...
          elsif params[:reaffirmed]&.key?(:reaffirmation_of)
        ...
        def render_draft(draft, _opts, params)
          ...
          result = if draft[:version].is_a?(Array)
                     ...
                     "/#{'D' if draft[:version].to_s.match?(/^\d/)}#{draft[:version]}"
                   end
          result += params[:iteration].to_s if params[:iteration]
        ...
        def render_amendment(amendment, _opts, _params)
          ...
          result += if amendment.length > 2
                      "#{amendment[1..-2].join(', ')}, and #{amendment[-1]}"
                    else
                      amendment.last.to_s
                    end
