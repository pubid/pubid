      ...
      def build_adoption(parsed_hash)
        ...
        if parsed_hash[:adoption_amendment]
          ...
          base_identifier = if adoption_org.start_with?("ISO") || adoption_org.start_with?("CEI")
                               PubidNew::Iso.parse(base_str)
                             elsif adoption_org.start_with?("IEC")
                               PubidNew::Iec.parse(base_str)
                             else
                               begin
                                 PubidNew::Iso.parse(base_str)
                               rescue PubidNew::Errors::ParseError
                                 PubidNew::Iec.parse(base_str)
                               end
                             end
          ...
          wrapped_identifier = if adoption_org.start_with?("ISO") || adoption_org.start_with?("CEI")
                                   PubidNew::Iso.parse(amendment_str)
                                 elsif adoption_org.start_with?("IEC")
                                   PubidNew::Iec.parse(amendment_str)
                                 else
                                   begin
                                     PubidNew::Iso.parse(amendment_str)
                                   rescue PubidNew::Errors::ParseError
          ...
          wrapped_identifier = if adoption_org.start_with?("ISO") || adoption_org.start_with?("CEI")
                                   PubidNew::Iso.parse(wrapped_id_str)
                                 elsif adoption_org.start_with?("IEC")
                                   PubidNew::Iec.parse(wrapped_id_str)
                                 else
                                   begin
                                     PubidNew::Iso.parse(wrapped_id_str)
                                   rescue PubidNew::Errors::ParseError
