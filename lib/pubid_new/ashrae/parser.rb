...
module PubidNew
  module Ashrae
    ...
    class Parser < Parslet::Parser
      ...
      # Optional suffix in parentheses (PDF), (I-P and SI versions), etc.
      rule(:optional_suffix) do
        space.maybe >> lparen >> (
          str("PDF") | # Special case for PDF
          (letter.repeat(1, 20) >> (space | digit | comma | dash | letter).repeat(0, 50)) # General case
        ) >> rparen
      end
      ...
      # Addendum code (single letter or multiple letters: a, b, ..., aa, ab, ..., aaa, etc.)
      rule(:addendum_code) do
        letter.repeat(1, 3).as(:addendum_code)
      end
      ...
      rule(:errata_suffix_on_addendum) do
        space >> str("Errata") >> space >> lparen >> month_name >> space >> digit.repeat(
          1, 2
